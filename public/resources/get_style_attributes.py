...
def get_style_attributes_svg(string):
    ...
    """
    local_styles = get_local_style_attributes(string)
    global_styles = get_global_style_attributes(string)
    global_group_styles = get_global_group_style_attributes(string)
    return combine_style_attributes(local_styles, global_styles, global_group_styles)


def get_style_attributes_path(string, animation_id, attribute):
...
def parse_svg(string):
    ...
    """
    paths, attrs = svg2paths2(string)
    return paths, attrs


def get_local_style_attributes(string):
    ...
    """
    return pd.DataFrame.from_records(_get_local_style_attributes(string))


def _get_local_style_attributes(string):
    try:
        _, attributes = parse_svg(string)
    except:
        print(f'{string}: Attributes not defined.')
    for i, attr in enumerate(attributes):
        ...
        yield dict(filename=string.split('.svg')[0], animation_id=animation_id, class_=class_, fill=fill, stroke=stroke,
                   stroke_width=stroke_width, opacity=opacity, stroke_opacity=stroke_opacity)


def get_global_style_attributes(string):
    ...
    """
    return pd.DataFrame.from_records(_get_global_style_attributes(string))


def _get_global_style_attributes(string):
    ...
    for i, attr in enumerate(style):
        ...
        for j in range(0, len(a.split(';}')) - 1):
            ...
            yield dict(filename=string.split('.svg')[0], class_=class_, fill=fill, stroke=stroke,
                       stroke_width=stroke_width, opacity=opacity, stroke_opacity=stroke_opacity)


def get_global_group_style_attributes(string):
    ...
    """
    df_group_animation_id_matching = pd.DataFrame.from_records(_get_group_animation_id_matching(string))

    df_group_attributes = pd.DataFrame.from_records(_get_global_group_style_attributes(string))
...
def _get_global_group_style_attributes(string):
    doc = minidom.parseString(string)
    ...
    for i, _ in enumerate(groups):
        ...
        yield dict(filename=string.split('.svg')[0], href=href.replace('#', ''), fill=fill, stroke=stroke,
                   stroke_width=stroke_width, opacity=opacity, stroke_opacity=stroke_opacity)


def _get_group_animation_id_matching(string):
    doc = minidom.parseString(string)
    try:
        ...
        for i, _ in enumerate(symbol):
            ...
            yield dict(filename=string.split('.svg')[0], href=href, animation_id=animation_id)
    except:
