...
def animate_logo(model_output: pd.DataFrame, logo_string: str):
    logo_xmin, logo_xmax, logo_ymin, logo_ymax = get_svg_bbox(logo_string)
    ...
    for animation_id in animations_by_id.keys():
        path_xmin, path_xmax, path_ymin, path_ymax = get_path_bbox(logo_string, animation_id)
            ...
            for i in range(len(animations_by_type[animation_type])):
                ...
                if animation_type == 1:
                    ...
                    if i < len(animations_by_type[animation_type]) - 1:
                        ...
                        fill_style = get_style_attributes_path(logo_string, animation_id, "fill")
                        stroke_style = get_style_attributes_path(logo_string, animation_id, "stroke")
...
def _insert_animations(animations: list, logo_string: str):
    print('Insert animations')
    # Load XML
    document = minidom.parseString(logo_string)
    ...
    return document.toprettyxml(encoding="iso-8859-1")
...
if __name__ == "__main__":
    ...
    # Process content
    processed_content = animate_logo(content)
