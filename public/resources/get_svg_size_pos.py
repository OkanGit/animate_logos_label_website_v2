...
def get_svg_bbox(string):
    ...
    try:
        ...
        print(f"{string}: svg2path fails. SVG bbox is computed by using get_svg_size. {e}")
        width, height = get_svg_size(string)
...
def get_path_bbox(string, animation_id):
    ...
    """
    try:
        paths, attributes = svg2paths2(string)
    except Exception as e1:
        print(f"{string}, animation ID {animation_id}: svg2path fails and path bbox cannot be computed. {e1}")
        return 0, 0, 0, 0
    ...
    for i, path in enumerate(paths):
        if attributes[i]["animation_id"] == str(animation_id):
            try:
                ...
                print(f"{string}, animation ID {animation_id}: svg2path fails and path bbox cannot be computed. {e2}")
                return 0, 0, 0, 0


def get_midpoint_of_path_bbox(string, animation_id):
    ...
    """
    try:
        xmin, xmax, ymin, ymax = get_path_bbox(string, animation_id)
        x_midpoint = (xmin + xmax) / 2
        y_midpoint = (ymin + ymax) / 2

        return x_midpoint, y_midpoint
    except Exception as e:
        print(f'Could not get midpoint for file {string} and animation ID {animation_id}: {e}')
        return 0, 0


def get_bbox_of_multiple_paths(string, animation_ids):
    ...
    try:
        ...
        print(f"{string}: svg2path fails and bbox of multiple paths cannot be computed. {e1}")
        return 0, 0, 0, 0
...
def get_relative_path_pos(string, animation_id):
    ...
    """
    path_midpoint_x, path_midpoint_y = get_midpoint_of_path_bbox(string, animation_id)
    svg_xmin, svg_xmax, svg_ymin, svg_ymax = get_svg_bbox(string)
...
def get_relative_pos_to_bounding_box_of_animated_paths(string, animation_id, animated_animation_ids):
    ...
    """
    path_midpoint_x, path_midpoint_y = get_midpoint_of_path_bbox(string, animation_id)
    xmin, xmax, ymin, ymax = get_bbox_of_multiple_paths(string, animated_animation_ids)
    try:
        rel_x_position = (path_midpoint_x - xmin) / (xmax - xmin)
    except Exception as e1:
        rel_x_position = 0.5
        print(f"{string}, animation_id {animation_id}, animated_animation_ids {animated_animation_ids}: rel_x_position not defined and set to 0.5. {e1}")
    try:
        rel_y_position = (path_midpoint_y - ymin) / (ymax - ymin)
    except Exception as e2:
        rel_y_position = 0.5
        print(f"{string}, animation_id {animation_id}, animated_animation_ids {animated_animation_ids}: rel_y_position not defined and set to 0.5. {e2}")

    return rel_x_position, rel_y_position


def get_relative_path_size(string, animation_id):
    ...
    path_xmin, path_xmax, path_ymin, path_ymax = get_path_bbox(string, animation_id)
...
def get_begin_values_by_starting_pos(string, animation_ids, start=1, step=0.5):
    ...
    for i in range(len(animation_ids)):
        x, _, _, _ = get_path_bbox(string, animation_ids[i])  # get x value of upper left corner
